...
export default function Static_6_5() {
  ...
  const RANGES = {
    ...
    caudal: { min: 8, max: 44 },//Q1 caudal: { min: 5, max: 30 },
  ...
  const handleConfirmCM = async () => {
    try{    
      ...
      const puedeAvanzar = parseInt(count_secuencia) === 5; 
      ...
      if (!puedeAvanzar) {
        ...
        localStorage.setItem("count_secuencia", "6");
        return true;
      }
    } catch (error) {
      console.error(error);
  ...
  const confirmationMessage = isOpenCustomMessage ? (
    <CustomAlert
      ...
      routeRedirect={"/client/Q2/static_7"} // No redirige automáticamente
